# Shia–Sunni Clarification Questions (for an Ahl-e-Hadith Group)
...
verbatim, with its full supporting proof/reference text and any images
directly underneath it, plus a `Source:` line pointing back to the exact
message in `source/raw-transcript.md`.
...
- **No question was reworded, and no reference or image was skipped.**
  ...
  only navigation text, headings, and cross-reference notes are original
  writing.
- Chapter groupings are an organizational choice (not present in the
  source) — the source itself has no chapters, only a chronological
  WhatsApp-style log. Where a single message contained more than one
  distinct question on different topics, it was split across chapters
  with a note, rather than force-fit into one.
- Where the source itself was incomplete or ambiguous (e.g. a screenshot
  ...
  source), that gap is flagged explicitly in the relevant chapter rather
